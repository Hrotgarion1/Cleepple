   ...
   <div class="container grid grid-cols-1 lg:grid-cols-3 gap-8">
      <div class="lg:col-span-2">
          ...
          <div class="flex items-center mt-4 cursor-pointer" wire:click="completed">
             ...
             @endif
                 <p class="text-sm ml-2">Marcar esta lección como realizada</p>
          </div>
          ...
          <div class="card mt-2">
             <div class="card-body flex text-gray-500 font-bold">
                ...
                @if ($this->previous)
                <a wire:click="changeLesson({{$this->previous}})" class="cursor-pointer">{{ __('Lección anterior')}}</a>    
                @endif

                @if ($this->next)
                <a wire:click="changeLesson({{$this->next}})" class="ml-auto cursor-pointer">{{ __('Siguiente lección')}}</a>
                @endif
      ...
      <div class="card">
         <div class="card-body">
            ...
            <p class="text-gray-600 text-sm mt-2">{{$this->advance . '%'}} {{ __('completado')}}</p>
